...
parser.add_argument("--ct_meanv", default=111.1232, type=float, help="ct mean, default 120.4875")
parser.add_argument("--ct_stdv", default=436.3707, type=float, help="ct std, default 403.3456")
parser.add_argument("--norm_param", default=2, type=int, help="normalization parameter for std, default 4")
parser.add_argument("--tissue_type_balanced_sampling", default=1, type=int, help="tissue type balanced sampling, default 0")

parser.add_argument("--patch_size", default=64, type=int, help="patch size, default 64")
...
def main():
    ...
    for filename in filenames_test:
        ...
        #####################################################################
        file_ct = filename.replace('mk.nii.gz', 'et.nii.gz');
        ...
        #####################################################################
        ct_out = predict_aver_res_fast(r1np, r2np, t1np, fa3e1np, fa3e2np, fa15e1np, fa15e2np, dixon1np, dixon2np, mknp, opt, dPatch, dTarget, step, netG, 5,opt.batchSize)
        ...
        # # ct_mean = 113.1875
        # # ct_std = 424.5913
        ct_mean = opt.ct_meanv 
        ct_std = opt.ct_stdv 
        ct_out_hu = ct_out*opt.norm_param*ct_std + ct_mean

        ct_out_hu[np.where(mknp==0)] = -1024
